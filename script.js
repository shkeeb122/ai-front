...
function restoreHistory(){
    ...
    conversationHistory.forEach(h=>{
        let roleClass = h.role==="user" ? "status-blue":"status-green";
        html += `<p><strong>${h.role}:</strong> <span class="${roleClass}">${h.content}</span></p>`;
    });
    html += "</div>";
...
async function viewHistory(campaignId){
    ...
    try{  
        const res = await fetch(`${API_URL}/conversation/${campaignId}`);  
        const data = await res.json();  
        ...
        data.messages.forEach(msg => {  
            let roleClass = msg.role==="user"?"status-blue":"status-green";  
            html += `<p><strong>${msg.role}:</strong> <span class="${roleClass}">${msg.content}</span><br><small>${new Date(msg.timestamp).toLocaleString()}</small></p><hr>`;  
        });  
        html += "</div>";  
